...
describe("/api/admin/tenants/[id]/billing", () => {
  beforeAll(async () => {
    ...
    const cto = await prisma.agencyStaff.create({
      data: { email: `billing-route-cto+${uniqueId}@fatoorasync.sa`, passwordHash: await hashPassword("x"), role: "CTO" },
    });
    ctoId = cto.id;
    const developer = await prisma.agencyStaff.create({
      data: { email: `billing-route-dev+${uniqueId}@fatoorasync.sa`, passwordHash: await hashPassword("x"), role: "DEVELOPER" },
    });
  ...
  it("performs a partial update, leaving trialEndsAt and featureFlags untouched when omitted", async () => {
    const before = await prisma.tenant.findUnique({
      where: { id: tenantId },
      select: { trialEndsAt: true, featureFlags: true },
    });

    const response = await PATCH(
      new Request("http://localhost", { method: "PATCH", body: JSON.stringify({ billingStatus: "SUSPENDED" }) }),
      { params: Promise.resolve({ id: tenantId }) }
    );
    expect(response.status).toBe(200);
    const body = await response.json();
    expect(body.billingStatus).toBe("SUSPENDED");
    expect(body.trialEndsAt).toBe(before?.trialEndsAt?.toISOString() ?? null);
    expect(body.featureFlags).toEqual(before?.featureFlags);
  });
