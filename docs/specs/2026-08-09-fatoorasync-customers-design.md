...
- **Empty state:** if there are zero non-walk-in customers (regardless of the inactive toggle), the table area is replaced with a centered message: "No customers yet — add your first one." No second "+ Add Customer" button here — the toolbar's button is never hidden, so it already covers this case; a second one would duplicate the page's one primary action, which §8's button placement rule (single location, never duplicated) rules out.
