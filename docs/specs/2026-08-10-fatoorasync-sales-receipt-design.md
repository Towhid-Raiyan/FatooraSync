...
A search input (SKU/barcode/name, client-side over the tenant's already-fetched active-product list) finds catalog items as the cashier types or scans. Selecting a result appends a line row: SKU, Product name, Unit Price, Qty (defaults to `1`, the only editable cell), VAT, Line Total, and a delete action. Unit Price and VAT are **not** editable per line — the UI displays them read-only from the product's catalog values at the moment it's added, so the cashier sees a stable price while building the receipt.
...
The request body carries only `{ customerId?, newCustomer?, lines: [{ productId, quantity }], notes? }` — no line-level price/VAT/name fields (§5's trust-boundary note).

1. **Resolve the customer** — either the selected existing customer's id, or (if `newCustomer` was provided) create the new `Customer` row first, inside this same transaction.
2. **Resolve each line's product server-side**: for every `{ productId, quantity }`, `tx.product.findUnique({ where: { id: productId } })` (tenant-scoped via `withTenant`, so a `productId` from another tenant simply isn't found — see §12) and compute `lineSubtotal`/`lineVat`/`lineTotal` (§7) from *that* fresh read's `nameEn`/`unitPrice`/`vatRate`, never from anything in the request body.
3. **Atomically consume the next receipt number and read the prior hash in one call**: `tx.tenant.update({ where: { id: tenantId }, data: { nextSalesReceiptNumber: { increment: 1 } }, select: { nextSalesReceiptNumber: true, lastSalesReceiptHash: true } })`. Because `lastSalesReceiptHash` isn't part of `data` in this call, the `select` returns its value as it stood *before* this update — a single atomic read-and-increment, no separate query, no race window. This `UPDATE` also takes a row lock on the `Tenant` row for the rest of the transaction, which is what actually makes the numbering gapless under concurrency: a second save request for the same tenant blocks here until the first transaction commits or rolls back, so two receipts can never be issued with the same number, and a save that fails partway through never leaves the counter incremented without a matching `Document` row, because the whole transaction rolls back together.
...
5. **Create the `Document` and its `DocumentLine` rows** — each line stores the server-read `productName`/`unitPrice`/`vatRate` from step 2.
6. **Decrement `Product.quantity`** for every line, by that line's quantity (allowed to go negative, per §5).
7. **Write the new hash back**: `tx.tenant.update({ where: { id: tenantId }, data: { lastSalesReceiptHash: newHash } })`.
