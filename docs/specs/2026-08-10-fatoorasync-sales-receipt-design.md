...
The request body carries only `{ customerId?, newCustomer?, lines: [{ productId, quantity }], notes? }` — no line-level price/VAT/name fields (§5's trust-boundary note). Inside the transaction, in order:

1. **Resolve the customer** — either `txn.customer.findFirst({ where: { id: customerId, tenantId } })` (explicit tenant filter; if not found → 400, see §12), or (if `newCustomer` was provided) `txn.customer.create({ data: { ..., tenantId } })` — explicit `tenantId` stamped on the create too, not left to auto-injection.
2. **Resolve each line's product server-side**: for every `{ productId, quantity }`, `txn.product.findFirst({ where: { id: productId, tenantId, isActive: true } })` — a `productId` from another tenant, or an inactive product, simply isn't found (400, see §12) — and compute `lineSubtotal`/`lineVat`/`lineTotal` (§7) from *that* fresh read's `nameEn`/`unitPrice`/`vatRate` (falling back to `Settings.defaultVatRate` when the product's own `vatRate` is `null`), never from anything in the request body.
