...
> **Superseded on SKU behavior (2026-08-10, after this plan shipped):** every mention of `sku` below as an optional, user-entered field — in the API bodies, the `findUniquenessConflict` helper, the dialog's SKU input, and the SKU-conflict tests — describes the *original* behavior only. It was changed after this plan's 3 tasks were merged: SKU is now always system-generated on creation (a per-tenant sequential counter) and is never accepted from the client or editable afterward. This plan is a historical record of what was originally built, not a live spec — do not execute or copy its SKU-related code verbatim. The current source of truth is `docs/specs/2026-08-10-fatoorasync-products-design.md`, whose own revision note explains the change.
