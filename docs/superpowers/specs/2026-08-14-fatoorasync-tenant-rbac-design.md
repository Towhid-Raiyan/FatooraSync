...
Tier 2's `cashierCanManageCatalog` toggle only governs the dedicated Products/Customers management routes listed above. It does not reach into `/api/receipts` or `/api/quotations`, which this plan never touches: saving a receipt or quotation can find-or-create a `Customer`, reactivate a deactivated one, and decrement `Product.quantity`, all as an incidental side effect of completing the sale. Those writes are Tier 1 territory — a Cashier always needs to complete a sale regardless of how the Owner has set the catalog-management toggle — so they stay ungated by design, not by oversight.
