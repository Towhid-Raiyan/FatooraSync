...
- **No email normalization anywhere in the `AgencyStaff` auth path** — neither the seed script nor `authorize()` lowercases or otherwise transforms the email before matching. This mirrors the exact bug already found and fixed on the tenant `User` side (RBAC branch: `POST /api/users` lowercased on create, `authorize()` matched exact-case on login, silently bricking new Cashier logins) — the fix there was consistency through *no* normalization on either side, and this new auth path must follow the same resolved pattern rather than reintroducing the asymmetry.
